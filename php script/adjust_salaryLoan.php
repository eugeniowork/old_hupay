...
if (isset($_POST["adjust_remainingBalance"]) && isset($_POST["adjust_cashPayment"]) && isset($_POST["adjust_newRemainingBalance"])
	...
	$salaryLoanId = $_POST["adjust_salaryLoanId"];

	
	// for validation
	// if the cash payment is greater than remaining balance
	if ($cashPayment > $remainingBalance) {
		$_SESSION["adjustment_salaryLoan_error"]= "<center><span class='glyphicon glyphicon-remove' style='color:#b03a2e'></span> The <b>Cash Payment Php ".$money_class->getMoney($cashPayment)."</b> must be not greater than the <b>Outstanding Balance Php ".$money_class->getMoney($remainingBalance)."</b>.</center>";
	}

	// if 0 the cash payment
	if ($cashPayment == 0) {
		$_SESSION["adjustment_salaryLoan_error"]= "<center><span class='glyphicon glyphicon-remove' style='color:#b03a2e'></span> The <b>Cash Payment</b> must be not equal to <b>0</b>.</center>";
	}
	...
	// if success
		...
		$salary_loan_class->updateOnlyRemainingBalance($salaryLoanId,$newRemainingBalance);

		$emp_id = $salary_loan_class->getInfoBySalaryLoanId($salaryLoanId)->emp_id;
		$pagibig_loan_id = 0;
		//$sss_loan_id = 0;
		$sssLoanId = 0;
		...
		$_SESSION["adjustment_salaryLoan_success"] = "<center><span class='glyphicon glyphicon-ok' style='color:#1d8348'></span> <b>Pag-ibig Loan</b> of  <b>Employee $fullName</b> is successfully adjusted.</center>";
