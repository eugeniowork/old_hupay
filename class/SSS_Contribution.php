<?php
class SSS_Contribution extends Connect_db{


    public function alreadyData(){
         $connect = $this->connect();
        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_sss_contrib_table"));
        return $num_rows;
    }
    ...
    public function getInfoByContribId($sss_contrib_id){
        $connect = $this->connect();

        $sss_contrib_id = mysqli_real_escape_string($connect,$sss_contrib_id);

        $select_qry = "SELECT * FROM tb_sss_contrib_table WHERE sss_contrib_id = '$sss_contrib_id'";
	...
	public function insertSSScontribution($compensationFrom,$compensationTo,$contribution,$dateCreated){
		$connect = $this->connect();
		...
		$insert_qry = "INSERT INTO tb_sss_contrib_table (sss_contrib_id,compensationFrom,compensationTo,Contribution,DateCreated) VALUES ('','$compensationFrom','$compensationTo','$contribution','$dateCreated')";
	...
	public function existContribution($compensationFrom,$compensationTo,$contribution){
		...
		$num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_sss_contrib_table WHERE compensationFrom = '$compensationFrom' AND compensationTo = '$compensationTo' AND contribution = '$contribution'"));
		return $num_rows;
	}
	...
	public function getContributionToTable(){
		$connect = $this->connect();
		$select_qry = "SELECT * FROM tb_sss_contrib_table ORDER BY Contribution ASC";
		if ($result = mysqli_query($connect,$select_qry)){
			while ($row = mysqli_fetch_object($result)){
				...
				echo "<tr id='".$row->sss_contrib_id."'>";
					echo "<td>" .$compensationFrom. " - ". $this->getMoney($row->compensationTo) ."</td>";
					echo "<td>" .$this->getMoney($row->Contribution)."</td>";
					echo "<td>";

                    if ($_SESSION["role"] == 1){
							echo "<span class='glyphicon glyphicon-pencil' style='color:#b7950b'></span> <a href='#' id='edit_sss_contrib' class='action-a'>Edit</a>";
							echo "<span> | </span>";
							echo "<span class='glyphicon glyphicon-trash' style='color:#515a5a'></span> <a href='#' id='delete_sss_contrib' class='action-a'>Delete</a>";
					}
                    else {
                        echo "No action";
                    }
    ...
    public function existOver(){
        $connect = $this->connect();
        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_sss_contrib_table WHERE compensationTo = '0'"));
        return $num_rows;
    }


    // for checking if exist sss contribution ID
   public function existContributionId($sss_contrib_id){
        $connect = $this->connect();

        $sss_contrib_id = mysqli_real_escape_string($connect,$sss_contrib_id);

        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_sss_contrib_table WHERE sss_contrib_id = '$sss_contrib_id'"));
        return $num_rows;
    }


     // for checking if exist sss contribution ID
   public function sameContributionInfo($sss_contrib_id,$compensationFrom,$compensationTo,$contribution){
        $connect = $this->connect();
        ...
        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_sss_contrib_table WHERE compensationFrom = '$compensationFrom' AND compensationTo = '$compensationTo' AND contribution = '$contribution' AND sss_contrib_id='$sss_contrib_id'"));
        return $num_rows;
    }


    // for update contribution info
    public function updateContributionInfo($sss_contrib_id,$compensationFrom,$compensationTo,$contribution){
        $connect = $this->connect();
        ...
        $update_qry = "UPDATE tb_sss_contrib_table SET compensationFrom = '$compensationFrom', compensationTo='$compensationTo', Contribution = '$contribution' WHERE sss_contrib_id='$sss_contrib_id'";
        $sql = mysqli_query($connect,$update_qry);
    }


    // delete contribution info
    public function deleteContributionInfo($sss_contrib_id){
        $connect = $this->connect();

        $sss_contrib_id = mysqli_real_escape_string($connect,$sss_contrib_id);
        $update_qry = "DELETE FROM tb_sss_contrib_table WHERE sss_contrib_id='$sss_contrib_id'";
        $sql = mysqli_query($connect,$update_qry);
    ...
    // check the last contribution to
     public function checkLastContributionTo(){
        ...
        $select_qry = "SELECT * FROM tb_sss_contrib_table ORDER BY compensationTo ASC LIMIT 1";
        $result = mysqli_query($connect,$select_qry);
        $row = mysqli_fetch_object($result);
    ...
    public function getContribution($salary){
        ...
        if ($date_payroll == "30" || $date_payroll == "29" || $date_payroll == "28") {
           // echo $salary;
            $select_qry = "SELECT * FROM tb_sss_contrib_table";
            if ($result = mysqli_query($connect,$select_qry)){
                while($row = mysqli_fetch_object($result)){
