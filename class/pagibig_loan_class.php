<?php
class PagibigLoan extends Connect_db{

	
	public function getInfoByPagibigLoanId($pagibig_loan_id){
		$connect = $this->connect();

		$pagibig_loan_id = mysqli_real_escape_string($connect,$pagibig_loan_id);

		$select_qry = "SELECT * FROM tb_pagibig_loan WHERE pagibig_loan_id='$pagibig_loan_id'";
	...
	public function getInfoByPagibigLoanEmpId($emp_id){
		$connect = $this->connect();

		$emp_id = mysqli_real_escape_string($connect,$emp_id);

		$select_qry = "SELECT * FROM tb_pagibig_loan WHERE emp_id='$emp_id' ORDER BY pagibig_loan_id DESC LIMIT 1";
		$result = mysqli_query($connect,$select_qry);
		$row = mysqli_fetch_object($result);
	...
	public function getPagibigLoanToPayroll($emp_id){
		$connect = $this->connect();
		...
		$select_qry = "SELECT * FROM tb_pagibig_loan WHERE emp_id='$emp_id'";
				...
				if ($row->dateFrom <= $date_payroll){
					...
					if ($row->dateFrom <= $date_payroll && $row->dateTo >= $date_payroll) { // july 30, 2017 , august 26, 2017 {}
						...
						if ($row->deduction >= $row->remainingBalance){
							$pagibig_loan_amount = $row->remainingBalance;
						}

						else {
							$pagibig_loan_amount = $row->deduction;
						}
					}// end of if
				}
	...
	public function insertPagibigLoan($emp_id,$dateFrom,$dateTo,$amountLoan,$deduction,$remainingBalance,$dateCreated){
		$connect = $this->connect();
		...
		$insert_qry = "INSERT INTO tb_pagibig_loan (pagibig_loan_id,emp_id,dateFrom,dateTo,amountLoan,deduction,remainingBalance,DateCreated)
												VALUES ('','$emp_id','$dateFrom','$dateTo','$amountLoan','$deduction','$remainingBalance','$dateCreated')";
		$sql = mysqli_query($connect,$insert_qry);
	}

	public function getPagibigLoanInfoToTable(){
		$connect = $this->connect();

		$select_qry = "SELECT * FROM tb_pagibig_loan";
		if ($result = mysqli_query($connect,$select_qry)){
			while ($row = mysqli_fetch_object($result)){
				...
				if ($row->remainingBalance != 0) {
					echo "<tr id='".$row->pagibig_loan_id."'>";
						...
						echo "<td><small>";
							echo "<span class='glyphicon glyphicon-pencil' style='color:#b7950b'></span> <a href='#' id='edit_pagibigLoan' class='action-a'>Edit</a>";
							echo "<span> | </span>";
							echo "<span class='glyphicon glyphicon-adjust' style='color: #239b56 '></span> <a href='#' id='adjust_pagibigLoan' class='action-a'>Adjustment</a>";
							echo "<span> | </span>";
							echo "<span class='glyphicon glyphicon-trash' style='color:#515a5a'></span> <a href='#' id='delete_pagibigLoan' class='action-a'>Delete</a>";
						echo "</small></td>";
					echo "</tr>";
				}
	...
	public function checkExistPagibigLoanUpdate($pagibig_loan_id){
		$connect = $this->connect();
		$pagibig_loan_id = mysqli_real_escape_string($connect,$pagibig_loan_id);

		$num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_pagibig_loan WHERE pagibig_loan_id = '$pagibig_loan_id'"));
		return $num_rows;
	}


	// check if has an existing loan
	public function existPagibigLoan($emp_id){
		$connect = $this->connect();
		$emp_id = mysqli_real_escape_string($connect,$emp_id);

		$num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_pagibig_loan WHERE emp_id = '$emp_id' AND remainingBalance != '0'"));
		return $num_rows;
	}


	// if no changes was made same info
	public function samePagibigLoanInfo($pagibig_loan_id,$dateFrom,$dateTo,$amountLoan,$deduction,$remainingBalance){
		$connect = $this->connect();
		...
		$num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_pagibig_loan WHERE 
												dateFrom = '$dateFrom' AND dateTo = '$dateTo' AND amountLoan = '$amountLoan' AND
												deduction = '$deduction' AND remainingBalance = '$remainingBalance' AND pagibig_loan_id = '$pagibig_loan_id'"));
		return $num_rows;
	}


	// for update information
	public function updatePagibigLoan($pagibig_loan_id,$dateFrom,$dateTo,$amountLoan,$deduction,$remainingBalance){
		$connect = $this->connect();
		...
		$update_qry = "UPDATE tb_pagibig_loan SET dateFrom = '$dateFrom',dateTo = '$dateTo', amountLoan = '$amountLoan', 
	...
	public function deletePagibigLoan($pagibig_loan_id){
		$connect = $this->connect();

		$pagibig_loan_id = mysqli_real_escape_string($connect,$pagibig_loan_id);

		$delete_qry = "DELETE FROM tb_pagibig_loan WHERE pagibig_loan_id = '$pagibig_loan_id'";
	...
	public function existPendingPagibigLoan($emp_id){
		$connect = $this->connect();
		$emp_id = mysqli_real_escape_string($connect,$emp_id);
		...
		$num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_pagibig_loan WHERE dateFrom <= '$date_payroll' AND dateTo >= '$date_payroll' AND emp_id = '$emp_id' AND remainingBalance != '0'"));
		return $num_rows;
	}
	...
	public function deductPagibigLoan(){
		$connect = $this->connect();
		...
		$select_qry = "SELECT * FROM tb_pagibig_loan WHERE remainingBalance != '0'";
		if ($result = mysqli_query($connect,$select_qry)){
			while ($row = mysqli_fetch_object($result)){
				...
				//if ($row->dateFrom <= $date_payroll && $row->dateTo >= $date_payroll) { // july 30, 2017 , august 26, 2017 {}
					...
					$remainingBalance = $row->remainingBalance - $row_payroll_info->pagibigLoan;
					// so update qry for new remaining balance
					$update_qry = "UPDATE tb_pagibig_loan SET remainingBalance = '$remainingBalance' WHERE pagibig_loan_id = '$row->pagibig_loan_id'";
					$sql = mysqli_query($connect,$update_qry);
				//}
	...
	public function updateOnlyRemainingBalance($pagibig_loan_id,$remainingBalance){
		$connect = $this->connect();

		$pagibig_loan_id = mysqli_real_escape_string($connect,$pagibig_loan_id);
		$remainingBalance = mysqli_real_escape_string($connect,$remainingBalance);

		$update_qry = "UPDATE tb_pagibig_loan SET remainingBalance = '$remainingBalance' WHERE pagibig_loan_id = '$pagibig_loan_id'";
		$sql = mysqli_query($connect,$update_qry);
	}
	...
	public function getPagibigLoanInfoHistoryToTable($emp_id){
		$connect = $this->connect();

		$emp_id = mysqli_real_escape_string($connect,$emp_id);

		$select_qry = "SELECT * FROM tb_pagibig_loan WHERE emp_id = '$emp_id' ORDER BY DateCreated DESC";
		if ($result = mysqli_query($connect,$select_qry)){
			while ($row = mysqli_fetch_object($result)){
