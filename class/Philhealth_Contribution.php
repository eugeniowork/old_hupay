<?php
class Philhealth_Contribution extends Connect_db{


    public function alreadyData(){
         $connect = $this->connect();
        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_philhealth_contrib_table"));
        return $num_rows;
    }


	// get info by contribution id
    public function getInfoByContribId($philhealth_contrib_id){
        $connect = $this->connect();

        $philhealth_contrib_id = mysqli_real_escape_string($connect,$philhealth_contrib_id);

        $select_qry = "SELECT * FROM tb_philhealth_contrib_table WHERE philhealth_contrib_id = '$philhealth_contrib_id'";
	...
	public function insertPhilhealthcontribution($compensationFrom,$compensationTo,$contribution,$dateCreated){
		$connect = $this->connect();
		...
		$insert_qry = "INSERT INTO tb_philhealth_contrib_table (philhealth_contrib_id,compensationFrom,compensationTo,Contribution,DateCreated) VALUES ('','$compensationFrom','$compensationTo','$contribution','$dateCreated')";
	...
	public function existContribution($compensationFrom,$compensationTo,$contribution){
		...
		$num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_philhealth_contrib_table WHERE compensationFrom = '$compensationFrom' AND compensationTo = '$compensationTo' AND contribution = '$contribution'"));
		return $num_rows;
	}
	...
	public function getContributionToTable(){
		$connect = $this->connect();
		$select_qry = "SELECT * FROM tb_philhealth_contrib_table ORDER BY Contribution ASC";
		if ($result = mysqli_query($connect,$select_qry)){
			while ($row = mysqli_fetch_object($result)){
				...
				echo "<tr id='".$row->philhealth_contrib_id."'>";
					echo "<td>" .$compensationFrom. " - ". $this->getMoney($row->compensationTo) ."</td>";
					echo "<td>" .$this->getMoney($row->Contribution)."</td>";
					echo "<td>";
                            if ($_SESSION["role"] == 1){
    							echo "<span class='glyphicon glyphicon-pencil' style='color:#b7950b'></span> <a href='#' id='edit_philhealth_contrib' class='action-a'>Edit</a>";
    							echo "<span> | </span>";
    							echo "<span class='glyphicon glyphicon-trash' style='color:#515a5a'></span> <a href='#' id='delete_philhealth_contrib' class='action-a'>Delete</a>";
					       }
                           else {
                                echo "No action";
                           }
	...
	// check the last contribution to
     public function checkLastContributionTo(){
        ...
        $select_qry = "SELECT * FROM tb_philhealth_contrib_table ORDER BY compensationTo ASC LIMIT 1";
        $result = mysqli_query($connect,$select_qry);
        $row = mysqli_fetch_object($result);
    ...
    public function existOver(){
        $connect = $this->connect();
        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_philhealth_contrib_table WHERE compensationTo = '0'"));
        return $num_rows;
    }


     // for checking if exist sss contribution ID
   public function existContributionId($philhealth_contrib_id){
        $connect = $this->connect();

        $philhealth_contrib_id = mysqli_real_escape_string($connect,$philhealth_contrib_id);

        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_philhealth_contrib_table WHERE philhealth_contrib_id = '$philhealth_contrib_id'"));
        return $num_rows;
    }


      // for checking if exist sss contribution ID
   public function sameContributionInfo($philhealth_contrib_id,$compensationFrom,$compensationTo,$contribution){
        $connect = $this->connect();
        ...
        $num_rows = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tb_philhealth_contrib_table WHERE compensationFrom = '$compensationFrom' AND compensationTo = '$compensationTo' AND contribution = '$contribution' AND philhealth_contrib_id='$philhealth_contrib_id'"));
        return $num_rows;
    }


     // for update contribution info
    public function updateContributionInfo($philhealth_contrib_id,$compensationFrom,$compensationTo,$contribution){
        $connect = $this->connect();
        ...
        $update_qry = "UPDATE tb_philhealth_contrib_table SET compensationFrom = '$compensationFrom', compensationTo='$compensationTo', Contribution = '$contribution' WHERE philhealth_contrib_id='$philhealth_contrib_id'";
    ...
    public function deleteContributionInfo($philhealth_contrib_id){
        $connect = $this->connect();

        $philhealth_contrib_id = mysqli_real_escape_string($connect,$philhealth_contrib_id);
        $update_qry = "DELETE FROM tb_philhealth_contrib_table WHERE philhealth_contrib_id='$philhealth_contrib_id'";
        $sql = mysqli_query($connect,$update_qry);
     ...
     public function getContribution($salary){
        ...
        /*
        if ($date_payroll == "15") { 
            $select_qry = "SELECT * FROM tb_philhealth_contrib_table";
            if ($result = mysqli_query($connect,$select_qry)){
                while($row = mysqli_fetch_object($result)){
