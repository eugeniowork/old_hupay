...
?>
	...
	<div class="col-sm-12" style="margin-top:50px;border:1px solid #BDBDBD;">
		...
		<table id="attendance_list" class="table table-bordered table-hover table-striped" style="border:1px solid #BDBDBD;">
			...
			<tbody>	
				<?php
					$attendance_class->getAllAttendanceToTable($dateFrom,$dateTo);
		...
		<div class="pull-right" id="print_late_attendance_list" style="cursor:pointer;margin-bottom:10px;">
			<?php
				$from = date_format(date_create($dateFrom), 'F d');
				$to = date_format(date_create($dateTo), 'F d, Y');
			?>
 			<span class="glyphicon glyphicon-print" style="color: #2c3e50"></span> <b style="color:#158cba">Print Late Attendance Reports <!--<?php echo $from . " - " . $to;?>--></b> 
 		</div>

 		


		<form id="form_print_late_attendance_list" method="post" action="">

		</form>

		<div class="pull-right" id="print_absent_attendace_list" style="cursor:pointer;margin-bottom:10px;margin-right:10px">
			<?php
				$from = date_format(date_create($dateFrom), 'F d');
				$to = date_format(date_create($dateTo), 'F d, Y');
			?>
 			&nbsp;&nbsp;&nbsp;&nbsp;<span class="glyphicon glyphicon-print" style="color: #2c3e50"></span> <b style="color:#158cba">Print Absent Reports <!--<?php echo $from . " - " . $to;?> --></b>
 		</div>

 		<form id="form_print_absent_attendance_list" method="post" action="">
	...
	<script>
		$(document).ready(function(){
	       ...
	       $("div[id='print_late_attendance_list']").on("click",function() {
	       		var dateFrom = "<?php echo $dateFrom; ?>";
	       		var dateTo = "<?php echo $dateTo; ?>";

	       		
		        //window.location = "print_search_attendance_list.php";
		        $("#form_print_late_attendance_list").html('<input type="hidden" name="dateFrom" value="'+dateFrom+'"/><input type="hidden" name="dateTo" value="'+dateTo+'"/>');
		        $("#form_print_late_attendance_list").attr("action","print_late_attendance_list.php");
		        $("#form_print_late_attendance_list").submit();
		    });


	       // print_late_attendance_list
	       $("div[id='print_absent_attendace_list']").on("click",function() {
	       		var dateFrom = "<?php echo $dateFrom; ?>";
	       		var dateTo = "<?php echo $dateTo; ?>";

	       		
		        //window.location = "print_search_attendance_list.php";
		        $("#form_print_absent_attendance_list").html('<input type="hidden" name="dateFrom" value="'+dateFrom+'"/><input type="hidden" name="dateTo" value="'+dateTo+'"/>');
		        $("#form_print_absent_attendance_list").attr("action","print_absent_attendace_list.php");
		        $("#form_print_absent_attendance_list").submit();
		    });
		});
