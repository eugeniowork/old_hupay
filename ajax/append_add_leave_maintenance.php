		...
		<form class="form-horizontal" id="form_leave_maintenance">
			<div class="form-group">
				...
				<div class="col-md-6">
					...
					<select name="validation" class="form-control" required="required">
						...
						<?php
							$leave_class->getLeaveValidationToDropdown(0,"Add");
						?>
					</select>
				</div>
			</div>
			<div class="form-group">
				<div class="col-md-6">
					...
					<input type="text" class="form-control" name="no_days_to_file" required="required" id="number_only" disabled="disabled"/>
				</div>
				<div class="col-md-6">
					<label class="control-label">Leave Count</label>
					<input type="text" class="form-control" name="leave_count" required="required" id="number_only"/>
				</div>
			</div>
			...
			<div class="form-group">
				<div class="col-md-12">
					<div class="checkbox">
					  <label><input name="is_convetable_to_cash" type="checkbox" value="">Is Convertable To Cash</label>
					</div>
				</div>
			</div>
	...
	$(document).ready(function(){
        ...
        $("#form_leave_maintenance").on("click","button",function(){
        	...
        	if (name != "" && validation != "" && (((validation == 1 || validation == 2) && no_days_to_file != "") || ((validation == 3 || validation == 4) && no_days_to_file == "")) && leave_count != ""){
        		...
        		$.ajax({
		            type: "POST",
		            url: "php script/add_leave_maintenance.php",
		            data: datastring,
		            cache: false,
		           // datatype: "php",
